...
facebookRouter.use((0, express_session_1.default)({ secret: APP_SECRET, resave: true, saveUninitialized: true }));
// Initialize Passport
...
facebookRouter.get('/callback', passport_1.default.authenticate('facebook', { failureRedirect: '/users/auth/facebook/login' }), (req, res) => {
    const user = req.user;
    // res.json({ user })
    res.redirect(`http://localhost:3000/profile?user=${encodeURIComponent(JSON.stringify(user))}`);
});
// Logout route
